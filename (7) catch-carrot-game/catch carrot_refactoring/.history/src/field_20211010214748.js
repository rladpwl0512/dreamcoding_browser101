...
export default class Field {
  constructor(carrotCount, bugCount) {
    ...
    this.fieldRect = this.field.getBoundingClientRect(); //필드의 전체적 사이즈, 포지션 알 수 있다(필드 범위 내에서 벌레, 당근 위치해야하기 때문에 필요함) 
    this.field.addEventListener('click', this.onClick());
  }
  init() {
    this.field.innerHTML = ''; //새롭게 리셋되면서 추가
    this._addItem('carrot', CARROT_COUNT, 'img/carrot.png', 80); //carrot.png을 5개
    this._addItem('bug', BUG_COUNT, 'img/bug.png', 50);
  }
