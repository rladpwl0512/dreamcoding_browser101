...
class Game {
  ...
  stop() {
    ...
    this.onGameStop && this.onGameStop(Reason.cancel); //게임이 멈춘 이유 설명(cancel)
  }
  //this.finish()
  finish(win) {
    ...
    this.stopGameTimer();
    sound.stopBackground();
    this.onGameStop && this.onGameStop(win? Reason.win : Reason.lose); //게임이 멈춘 이유 설명(win, lose)
  }
  ...
  onItemClick = (item) => {
    ...
    if(item === 'carrot') {
      ...
      if(this.score === this.carrotCount) {
        this.finish(true); //이겼으니까 true 넘김(좋지않은코드, 이후 리팩토링)
      }
    }  else if(item === 'bug') {
      this.finish(false); //졌으니까 false 넘김 
    }
  };
  ...
  startGameTimer() {
    ...
    this.timer = setInterval(() => {
      if(remainingTimeSec <= 0) {
        ...
        this.finish(this.carrotCount === this.score); //지정된 시간안에 당근 먹으면 won, 아니라면 lost
        return; 
      }
      this.updateTimerText(--remainingTimeSec);
    }, 1000);
